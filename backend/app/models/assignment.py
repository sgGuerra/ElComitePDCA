...
async def assign_leader_to_process(assignment_data: AssignmentCreate, admin_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.exception(f"Error assigning leader to process: {e}")
        raise
...
async def remove_leader_from_process(process_id: int, leader_id: int) -> bool:
    ...
    try:
        ...
        logger.exception(f"Error removing leader from process: {e}")
        raise
    ...
    try:
        ...
        for leader in leaders:
            ...
            if 'created_at' not in leader or leader['created_at'] is None:
                leader['created_at'] = datetime.now(timezone.utc).replace(tzinfo=None)
            if 'updated_at' not in leader or leader['updated_at'] is None:
                leader['updated_at'] = datetime.now(timezone.utc).replace(tzinfo=None)
        
        return leaders
    except Exception as e:
        logger.exception(f"Error getting process leaders: {e}")
        raise
...
async def get_leader_processes(leader_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception(f"Error getting leader processes: {e}")
        raise
...
async def transfer_process_leadership(process_id: int, old_leader_id: int, new_leader_id: int) -> bool:
    ...
    try:
        ...
        logger.exception(f"Error transferring process leadership: {e}")
        raise
